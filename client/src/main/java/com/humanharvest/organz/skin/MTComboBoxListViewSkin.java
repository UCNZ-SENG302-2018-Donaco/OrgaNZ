...
public class MTComboBoxListViewSkin<T> extends ComboBoxListViewSkin<T> {
    ...
    private BooleanProperty touchPressed = new BooleanPropertyBase(false) {
        protected void invalidated() {
            getPopupContent().pseudoClassStateChanged(PRESSED_PSEUDO_CLASS, get());
        }
    ...
    public MTComboBoxListViewSkin(ComboBox<T> comboBox) {
        super(comboBox);
        ...
        if (focusAreaNode != null) {
            ...
            getPopupContent().addEventHandler(TouchEvent.TOUCH_PRESSED, event -> touchPressed.setValue(true));
        }
    }
